...
constexpr Float ProjectionScale = 1.00f;
...
PBDSimulation2DExample::PBDSimulation2DExample(const Arguments& arguments): Platform::Application{arguments, NoCreate} {
    ...
    /* Setup window */
    {
        ...
        Configuration conf;
        conf.setTitle("PBD Granular Simulation")
...
void PBDSimulation2DExample::setupSolver(SDFObject::ObjectType boundaryType, bool firstTime) {
    ...
    sceneObjs->emitterT0 = SDFObject{ gridCenter() , 30.0f, SDFObject::ObjectType::Circle };
    //+ Vector2(10.0f, -5.0f)
    ...
    auto b_color = Color3{ 230.0f/255.0f, 142.0f/255.0f,54.0f/255.0f };
   //_drawableBoundary->setColor(0xffffff_rgbf); // Chage boundary color
   _drawableBoundary->setColor(b_color);
}
...
void PBDSimulation2DExample::drawEvent() {
    ...
    GL::defaultFramebuffer.clearColor(Color3{70.0f/255.0f, 30.0f/255.0f,82.0f/255.0f });
    /* Draw objects */
    {
        /* orient boundary */
        Matrix3 mat = Matrix3::rotation(Rad(_orient));
...
void PBDSimulation2DExample::keyPressEvent(KeyEvent& event) {
    ...
    switch(event.key()) {
        ...
        case KeyEvent::Key::B:
            ...
            break;
        case KeyEvent::Key::W:            
            _pbdSolver->emitParticles(gridCenter() + Vector2(Float(dis(gen)), 20.0f), 30.0f, SDFObject::ObjectType::Circle);
            break;
        case KeyEvent::Key::E:
